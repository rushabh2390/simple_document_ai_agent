...
st.set_page_config(
    page_title="LangGraph Multimodal AI Agent Hub", page_icon="🤖", layout="wide")
...
with main_left:
    ...
    with inspector_col:
        ...
        with inspector_container:
            if st.session_state.inspected_nodes:
                st.caption(
                    "Active Visual & Data Context retrieved by Agent Tool:")
                for index, match in enumerate(st.session_state.inspected_nodes):
                    with st.container(border=True):
                        ...
                        if match.get("table_path") and os.path.exists(match["table_path"]):
                            st.markdown(
                                "📊 **Extracted Structural Table Data:**")
                            try:
                        ...
                        if match.get("image_path") and os.path.exists(match["image_path"]):
                            st.markdown(
                                "🖼️ **Extracted Diagram / Figure Asset:**")
                            st.image(match["image_path"],
                                     use_container_width=True)
            else:
                st.info(
                    "💡 Any extracted tables, schemas, or diagram layouts selected by the agent tools will stack right here.")
            ...
            with status_placeholder.container():
                with st.status("🧠 Agent Evaluating & Planning...", expanded=True) as status:
                    ...
                    try:
                        ...
                        status.update(
                            label="❌ Execution Failed", state="error")
                        final_state = None
    ...
    if uploaded_files and st.button("🚀 Build Knowledge Base Data", use_container_width=True):
        ...
        active_chunk_overlap = st.session_state.get(
            "chunk_overlap_slider", 200)
            ...
            with st.spinner(f"Parsing and processing: {uploaded_file.name}..."):
                try:
                    ...
                    if chunks:
                        ...
                        st.success(
                            f"Successfully processed & indexed: {uploaded_file.name}")
...
with main_right:
    ...
    with settings_container:
        ...
        st.slider("Max Chunk Size", min_value=200, max_value=3000,
                  value=1000, step=50, key="chunk_size_slider")
        st.slider("Chunk Overlap Block", min_value=0, max_value=500,
                  value=200, step=10, key="chunk_overlap_slider")

        st.divider()
        st.markdown("##### 🎛️ Agent Parameters")
        st.slider("Database Chunks (Retrieval K)", min_value=1,
                  max_value=10, value=3, step=1, key="retrieval_k_slider")
        st.slider("Generation Temperature", min_value=0.0,
                  max_value=1.0, value=0.1, step=0.1, key="temp_slider")
        st.slider("Generation Top-K Window", min_value=1,
                  max_value=100, value=40, step=1, key="top_k_slider")
