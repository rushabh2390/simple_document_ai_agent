...
parser_engine = MultiModalDocumentParser(
    base_output_dir=STATIC_ASSET_DIR, batch_size=3)
db_engine = RAGDatabaseManager(
    ...
    for file in files:
        ...
        if file_ext == ".zip":
                        ...
                        if arch_ext in supported_formats:
                            with archive.open(arch_name) as extracted_file:
                                ...
                                chunks = parser_engine.parse_file(
                                    file_bytes, os.path.basename(arch_name))
                                if chunks:
                                    db_engine.insert_document_chunks(
                                        chunks, os.path.basename(arch_name))
                                    processed_count += 1
            except Exception as e:
                logger.error(f"Failed to process ZIP archive: {str(e)}")
                raise HTTPException(
                    status_code=400, detail=f"Failed to process ZIP archive: {str(e)}")
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to parse '{file.filename}': {str(e)}")
        else:
            raise HTTPException(
...
async def ask_rag(payload: QueryRequest):
    ...
    matched_nodes = db_engine.search_bm25(
    ...
    try:
        ...
        llm = ChatOllama(base_url=ollama_endpoint,
