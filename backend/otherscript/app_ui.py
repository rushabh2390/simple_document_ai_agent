...
with main_left:
            ...
            if st.session_state.inspected_nodes:
                ...
                for index, match in enumerate(st.session_state.inspected_nodes):
                    with st.container(border=True):
                        ...
                        if match.get("text"):
                            with st.expander(
                                "🔬 View Text / Schema Context", expanded=True
                            ):
                                st.write(match["text"])

                        if match.get("table_path") and os.path.exists(
                            match["table_path"]
                        ):
                        ...
                        if match.get("image_path") and os.path.exists(
                            match["image_path"]
                        ):
    ...
    if uploaded_files and st.button(
        "🚀 Build Knowledge Base Data", use_container_width=True
    ):
        ...
        for uploaded_file in uploaded_files:
            with st.spinner(f"Parsing and processing: {uploaded_file.name}..."):
                try:
                    ...
                    st.error(f"Failed parsing {uploaded_file.name}: {ex!s}")
